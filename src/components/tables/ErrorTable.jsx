...
const ErrorTable = () => {
  ...
  const totalData = 100; // Общее количество записей (получается из API)
  const itemsPerPage = 12; // Сколько записей на странице
  ...
  return (
    <div className="p-6 bg-[#e4ebf3] rounded-xl">
      <div>
        <table className="w-full bg-[#f9f9f9] rounded-3xl overflow-hidden border-separate border-spacing-y-3">
          {/* Заголовок таблицы */}
          <div className="px-6">
            <thead className="bg-[#f9f9f9] text-gray-600 uppercase text-sm leading-normal">
              <tr className="border-b border-[#F7F9FB]">
                ...
                <th className="py-2 px-2 text-end font-medium w-16"></th>
              </tr>
            </thead>

            {/* Тело таблицы */}
            <tbody className="text-gray-700 text-sm rounded-3xl overflow-hidden">
              {paginatedData.map((item, index) => (
                <tr
                  key={index}
                  className="rounded-3xl border group border-gray-300 transition transform cursor-pointer"
                  onClick={() => handleRowClick(item)}
                >
                  <td className="py-4 bg-white rounded-l-3xl px-2 text-center font-semibold w-8">
                    {(currentPage - 1) * itemsPerPage + index + 1}.
                  </td>
                  <td className="py-4 bg-white group-hover:text-blue-500 transition-colors duration-500 px-2 text-center font-semibold w-40">
                    {item.kadasterId}
                  </td>
                  <td className="py-4 bg-white px-2 text-center font-bold w-24">
                    {item.modda}
                  </td>
                  <td className="py-4 bg-white px-2 text-center w-32">
                    {item.region}
                  </td>
                  <td className="py-4 bg-white px-2 text-center w-32">
                    {item.district}
                  </td>
                  <td className="py-4 bg-white px-2 text-center w-52">
                    {item.address}
                  </td>
                  <td className="py-4 bg-white px-2 text-center w-44">
                    {item.spaceImageId}
                  </td>
                  <td className="py-4 bg-white px-2 text-center w-32">
                    {item.spaceImageDate}
                  </td>
                  <td className="py-4 bg-white px-2 text-center w-24">
                    {item.category}
                  </td>
                  <td className="py-4 bg-white px-2 text-center w-32">
                    {item.arrivalDate}
                  </td>
                  <td className="py-4 bg-white px-2 text-orange-500 font-semibold text-center w-24">
                    {item.deadline}
                  </td>
                  <td className="py-4 bg-white px-6 text-center">
                    {item.planPdf && (
                      <button
                        className="flex items-center gap-2"
                        onClick={() => downloadPdf(item.planPdf)}
                      >
                        <svg
                          width="24"
                          height="24"
                  ...
                  {/* Кнопка скачивания PDF "Hokim qarori", только если modda !== "7-modda" */}
                  <td className="py-4 bg-white px-6 text-center">
                    {item.decisionPdf ? (
                      <button
                        className="flex items-center gap-2"
                        onClick={() => downloadPdf(item.decisionPdf)}
                      >
                        <svg
                          width="24"
                          height="24"
                  ...
                  {/* Динамическое изменение цвета статуса */}
                  <td className="py-4 bg-white px-6 font-medium"></td>
                  <td className="justify-end bg-white rounded-r-3xl">
                    <ChevronRight />
                  </td>
                </tr>
              ))}
