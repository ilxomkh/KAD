...
const TableRole3 = () => {
  ...
  // Переход к детальному просмотру для агенства
        ...
        <table className="min-w-full bg-[#f9f9f9] rounded-t-3xl px-6 overflow-hidden border-separate border-spacing-y-3">
          ...
          <tbody className="text-gray-700 text-sm md:text-base">
            {paginatedData.map((item, index) => {
              // Используем buildingPresence для отображения статуса Qurilma
              const qurilmaStatus = item.buildingPresence ? "BOR" : "YO‘Q";
              return (
                <tr
                  key={index}
                ...
                >
                  ...
                  <td
                    className={`py-4 px-6 bg-white font-semibold ${
                      item.buildingPresence ? "text-green-500" : "text-red-500"
                    }`}
                  >
                    {qurilmaStatus}
                  </td>
