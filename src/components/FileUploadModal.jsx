...
function FileUploadModal({ isOpen, onClose, onUpload, cadasterId }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Важно, чтобы поле называлось так, как ожидает сервер
      // Предположим, сервер ждет "screenshot"
      ...
      // Сервер может вернуть JSON с данными об успешно загруженном файле
      // const result = await response.json();

      // Создаем временный URL для предпросмотра
      const fileURL = URL.createObjectURL(file);

      // Вызываем колбэк, чтобы показать фото в VerdictPage
      onUpload(fileURL);
    ...
    >
      <div className="bg-white p-6 rounded-2xl shadow-lg w-full sm:w-1/3">
        <div className="flex justify-between items-center">
          ...
          {/* Кнопка закрытия (можно добавить иконку X) */}
        </div>

        {/* Область для перетаскивания или выбора файла */}
